...
let ExpressAuth, getSession, props;
...
async function initServer() {
  ...
  app.post("/api/order/create", async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        res
          .status(500)
          .json({ status: "error", data: null, error: err.message });
      } finally {
        client.release();
      }
    } catch (err) {
  ...
  app.post("/api/handle_success", async (req, res) => {
    ...
    if (!isValid)
      return res.redirect(`${process.env.FRONTEND_URL}/order_failed`);
